...
def build_model(X, y):
    ...
    # define model
    model = Sequential()
    model.add(Conv1D(filters= 35, kernel_size = 3, activation = 'relu', input_shape=(X.shape[1], X.shape[2]))) #kernel_initializer=glorot_normal(seed=1), bias_initializer='zeros'
    ...
    #plot the model
    # plot_model(model, show_shapes = True, to_file='WISDM_1DCNN.png')
    return model
...
# cm_analysis(testy, pred_y, filename='WISDM_pics/with_weights/new_WISDM_CNN_CM.png',
#             labels=['Downstairs','Jogging','Sitting',
#                     'Standing','Upstairs','Walking'], figsize=(8,8))
#
# # list the performance in history
# print(history.history.keys())
# # plot the lost in history
# plt.figure(2)
